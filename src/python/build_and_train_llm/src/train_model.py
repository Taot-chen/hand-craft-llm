...
import torch
from modeling_tinyllm import Transformer, ModelArgs
...
# 数据配置
batch_size = 32  # 每个微批次的样本数量，如果使用梯度累积，实际批次大小将更大
...
iter_batches = partial(
    ...
    dataset_path="./TinyStories/TinyStories_all_data/"
)
...
def plot_loss(iter_num, loss):
    ...
    plt.xlim(min(iter_num), max(iter_num))
    plt.ylim(0, max(loss)*1.1)
    plt.xlabel('iter_num', fontsize=12)
...
while True:
    ...
    if iter_num % log_interval == 0 and master_process:
        ...
        if local_iter_num >= 5:  # 让训练循环先运行几个迭代再计算模型利用率
            ...
            running_flops = raw_model.caculate_flops_per_iter(batch_size * gradient_accumulation_steps, dt)
        print(
            f"{iter_num} | loss {lossf:.4f} | lr {lr:e} | {dt * 1000:.2f}ms | mfu {running_mfu * 100:.2f}% | flops {running_flops / 1e12} TFLOPS"
            # mfu 表示模型浮点运算利用率
        )
